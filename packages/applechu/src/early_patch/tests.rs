...
#[test]
fn custom_release_tag_uses_reserved_bucket() {
    // Given: 两个版本转换函数都只允许内置版本范围
    let mut image = vec![0x90; 128];
    image[16..47].copy_from_slice(&[
        0xE8, 0, 0, 0, 0, 0x8B, 0x00, 0x83, 0xF8, 0x15, 0x77, 0x08, 0x8B, 0x04, 0x85, 0, 0, 0, 0,
        0xC3, 0xB8, 0x17, 0, 0, 0, 0xC3, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC,
    ]);
    image[64..92].copy_from_slice(&[
        0x8B, 0x44, 0x24, 0x04, 0x83, 0xF8, 0x15, 0x77, 0x08, 0x8B, 0x04, 0x85, 0, 0, 0, 0, 0xC3,
        0xB8, 0x17, 0, 0, 0, 0xC3, 0xCC, 0xCC, 0xCC, 0xCC, 0xCC,
    ]);
    let memory = FakeMemory::new(image);

    // When: AppleChu 在游戏入口前改写版本槽位映射
    patches::apply_pre_tls(&memory, &config(""));

    // Then: 负数保留无效哨兵，自定义 ID 共用预留槽位
    let image = memory.image.borrow();
    assert_eq!(
        &image[23..47],
        &[
            0x85, 0xC0, 0x78, 0x10, 0x83, 0xF8, 0x16, 0x72, 0x03, 0x33, 0xC0, 0xC3, 0x8B, 0x04,
            0x85, 0, 0, 0, 0, 0xC3, 0x6A, 0x17, 0x58, 0xC3,
        ]
    );
    assert_eq!(
        &image[68..92],
