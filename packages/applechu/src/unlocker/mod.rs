...
unsafe fn install_file_size_hooks(api: &Api) {
    if let Some(original) = hook_iat_all_modules(
        ...
        api.log_info("unlocker: GetFileSize hooked");
    }
    ...
    if let Some(original) = hook_iat_all_modules(
        ...
        api.log_info("unlocker: GetFileSizeEx hooked");
    }
}
...
pub fn init(api: &Api, config: &UnlockerConfig) {
    ...
    if enabled.is_empty() {
        api.log_warn("unlocker: 所有解锁项均已关闭，跳过安装");
        return;
    }
...
fn load_modified(path: &str, filename: &str, rule_index: usize) -> Option<Vec<u8>> {
    ...
    let raw = match std::fs::read(path) {
        ...
        Err(error) => {
            warn_once(rule_index, &LOGGED_READ_FAIL, || {
                format!("unlocker: 读取 {} 失败: {}", filename, error)
            });
            return None;
        }
    };
    ...
    let Ok(text) = std::str::from_utf8(&raw) else {
        warn_once(rule_index, &LOGGED_NOT_UTF8, || {
            format!("unlocker: {} 不是合法 UTF-8，已放行读原文件", filename)
        });
        return None;
    };
...
fn open_path(irp: &Irp) -> Option<String> {
    winapi::cstr_to_string(irp.open_filename_a.cast())
        .or_else(|| winapi::wide_to_string(irp.open_filename_w))
}
