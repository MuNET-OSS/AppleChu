...
pub fn init(api: &Api, config: &Config) {
    ...
    unsafe {
        ...
        if user32 == 0 {
            api.log_warn("DPI 感知初始化跳过: user32.dll 未加载");
            return;
        }

        let proc = GetProcAddress(user32, b"SetProcessDpiAwarenessContext\0".as_ptr());
        if proc.is_null() {
            api.log_info("DPI 感知初始化跳过: 当前系统不支持 Per-Monitor V2");
            return;
        }
        ...
        if set_process_dpi_awareness_context(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2) != 0 {
            api.log_info("DPI 感知已启用: Per-Monitor V2");
        } else {
            api.log_warn("DPI 感知初始化失败: SetProcessDpiAwarenessContext 返回失败");
        }
    }
}
