...
fn patch_force_true<M: PatchMemory>(api: &M) -> PatchResult {
...
fn patch_branch_skip<M: PatchMemory>(api: &M) -> PatchResult {
...
fn patch_already_or_write<M: PatchMemory>(api: &M, addr: usize, patch: &[u8]) -> PatchResult {
...
fn log_result<M: PatchMemory>(api: &M, name: &str, result: PatchResult) {
    match result {
